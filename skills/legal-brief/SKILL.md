...
- **Как обновлять:** ты пишешь `state.json` напрямую (через Write) - это механическое ведение состояния, одно из разрешенных механических исключений из правила "не пишу файлы" (наряду с созданием папки `_legal_brief/` и логом `conversion_errors.txt`).
