...
2. Pins `CLAUDE_PLUGIN_DATA` to `~/.claude/plugins/data/legal-brief-codex` so jobs/broker state stays isolated from any other codex install on the machine.
...
Patching the on-disk JSON at `~/.claude/plugins/data/legal-brief-codex/state/<workspace-slug>/jobs/<task>.json` does **not** help — the broker holds state in memory via `broker.sock`, not on disk.
...
- State + jobs + broker.sock: `~/.claude/plugins/data/legal-brief-codex/state/<workspace-slug>/`
- Job log: `~/.claude/plugins/data/legal-brief-codex/state/<workspace-slug>/jobs/task-XXXX.log`
